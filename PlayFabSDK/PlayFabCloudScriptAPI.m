...
-(void) GetFunction:(CloudScriptGetFunctionRequest*)request success:(GetFunctionCallback)callback failure:(ErrorCallback)errorCallback withUserData:(NSObject*)userData
{
    ...
    NSString *jsonString = [request JSONStringWithClass:[CloudScriptGetFunctionRequest class]];
    ...
    [connection setCompletionBlock:^(id obj, NSError *err) {
        ...
        if (!err) {
            ...
            if (playfab_error != nil) {
                ...
                NSDictionary *class_data = [JSON valueForKey:@"data"];
                CloudScriptGetFunctionResult *model = [[CloudScriptGetFunctionResult new] initWithDictionary:class_data];
    ...
    [connection postURL:[NSString stringWithFormat:@"%@%@",[PlayFabClientAPI GetURL],@"/CloudScript/GetFunction"] body:jsonString authType:nil authKey:nil];
}
